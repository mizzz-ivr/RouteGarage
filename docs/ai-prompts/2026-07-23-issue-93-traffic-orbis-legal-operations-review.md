...
4. 交通情報・オービス情報について次をレビューする。
   ...
   - 提供停止、訂正、通報、監査
5. 各判断を以下のいずれか1つへ区分する。
   - 確定要件
   - 条件付き許容候補
   - MVP禁止
   - 要専門確認
6. MVPでの採用・未採用状態を判断区分とは別に管理する。
7. 法的助言や適法性の最終判断をしない。
8. 公的資料で確認できない事項を推測で許容しない。
9. 実装コード・技術選定を変更しない。
...
- 走行中注視・操作を誘発する能動通知は`MVP禁止`。
- 一般的な安全運転注意は`確定要件`として許容。
- 地域・路線単位の粗い表示は`条件付き許容候補`。
- 正確座標と接近音声通知の判断区分は`要専門確認`、MVP扱いは`未採用`。
- 移動式リアルタイム情報、ユーザー投稿型正確地点、回避ルートは`MVP禁止`。
- JARTIC一般ウェブページの無断再表示・転載・改変を禁止。
- 警察庁資料から地域重点を設定できることを確認したが、東京・千葉・神奈川の確認事例を全国一律の策定単位へ一般化しない。
- 更新時刻不明は原則`提供停止`。明示的な制限表示ルールがある場合は、通常の制限表示条件を満たす場合のみ候補とする。
- 取得失敗キャッシュは、通常の制限表示条件に加えてキャッシュ専用条件を全て満たす場合のみ候補とする。
- 判断不能時はNo-Go。
...
- 確認した地域事例を全国一律へ一般化していないか。
- 更新時刻不明と取得失敗キャッシュの条件が既存方針と整合しているか。

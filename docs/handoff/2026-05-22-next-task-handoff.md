...
## 次アクション候補
1. branch cleanup確認（PR #82対応branch、既存候補branch）の削除可否をGitHub上で確定。
2. Issue #81へ完了コメントを追加し、Issue駆動運用のトレーサビリティを補完。
3. 高リスク領域の先行Issueを1件選定して起票（優先候補: 交通情報/オービスの正確性限界表示と規約整合）。
4. 利用規約/プライバシーポリシー/問い合わせ/監査ログ運用の要件Issueを分割起票。

## branch cleanup
- `docs/current-status.md` / `docs/active-issues.md` には「PR #82対応branchの削除可否確認が必要」と記録済み。

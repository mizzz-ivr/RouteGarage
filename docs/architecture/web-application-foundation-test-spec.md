...
- UT-002 `src/app/page.test.tsx`: RouteGarage、説明、SafetyNotice、主要3領域を表示し、未実装routeを指すリンク・ボタン・CTAを配置しない
...
viewport 320x800で次を確認する。

- 横スクロールなし
- heading / SafetyNoticeが視認可能
- ドライブ / ガレージ / 振り返りの主要静的コンテンツが欠落しない
- 主要テキスト・操作要素がresponsive CSSにより意図せず非表示にならない

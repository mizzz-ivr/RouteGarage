...
- 開発手法: ウォーターフォール
- AI生成物: 人間レビュー必須
- 現在の主目的: Issue #121 / PR #123として、JARTIC静的レイヤーの保持期間・再確認期限・削除SLAの暫定基準をレビューする
...
- Issue #121: Open
- PR #123: Open / mergeable / レビュー待ち
- `main`比較: 6 commits / 6 files / behind 0
- 変更範囲: docsのみ
- 未解決review thread: 0件
...
- 表示可否、鮮度、検証、権利を独立状態として保持し、`STOPPED`を最優先にする。
- 情報本体表示中は出典、加工、対象年月、提供元更新時点、参考情報、安全注意を固定表示する。
- 投稿・編集・公開変更等は停車中利用を原則とし、走行状態不明時は制限側へ倒す。
- 正確位置・開始終了地点・生活拠点・走行履歴を公開前提で表示しない。
- Webでは外部キャプチャを完全に防止できる前提を置かず、通常表示をプライバシーセーフにする。
- 原本から派生物まで多対多系譜で追跡し、表示停止を物理削除より先に実施する。

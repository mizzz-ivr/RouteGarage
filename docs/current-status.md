...
- Issue #113: JARTIC静的レイヤーの出典・加工・鮮度表示要件を定義する
  ...
  - Status: Codex P1 / P2指摘対応中 / 人間・法務・運用・安全・プライバシー・アクセシビリティレビュー未完了
...
- 更新遅延・過去版でも、検証・権利・表示可否条件を満たさない場合は非表示。
...
- 走行状態を判定できない場合は走行中と同等の安全側制御を適用する。
...
- Codex P1指摘: 10件
- Codex P2指摘: 2件
- 合計12件を画面要件・Source of Truthへ反映中
- P1 10件: 修正・返信・thread解決済み
- P2 2件: 画面要件本文へ修正済み。Source of Truth同期・返信・thread解決を実施中

...
- 開発手法: ウォーターフォール
- 主なAI支援: Codex
- 現在の主目的: Issue #93として、交通情報・オービス情報の法務・運用レビューを実施する
...
- オービス情報の正確座標表示、接近通知、音声通知は法務・提供元・地域運用・ストア審査確認が未完了。
- 交通情報・オービス情報の採用データ提供元と個別利用条件が未確定。
- 鮮度閾値、検証基準、キャッシュ有効期限、制限表示条件が未確定。
- 提供停止、訂正、通報、監査、事故・苦情対応の責任者とSLAが未確定。
- 高リスク領域の設計判断に法務・運用観点の追加レビューが必要。
- PR #86、PR #88、PR #90、PR #92の作業branchは削除可否の確認が必要。
...
1. Issue #93で交通情報・オービス情報の法務・運用レビューを完了し、人間レビュー後にマージする。
2. データ提供元候補ごとの利用条件・上流由来・再提供経路を比較する。
3. 交通情報・オービス情報の表示粒度、注意文言、停止表示の画面詳細設計を行う。
4. 提供停止、訂正、通報、監査、事故・苦情対応の運用設計を行う。
5. 位置情報・走行履歴の公開制御、保持期間、削除導線を詳細化する。
6. 画像投稿・コミュニティ機能のモデレーション、通報、権利侵害対応要件を整理する。
7. 利用規約、プライバシーポリシー、問い合わせ導線、監査ログ運用要件を整理する。